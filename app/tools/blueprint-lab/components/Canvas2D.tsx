...
import {
  ...
  snapToLineFeatures, snapToWallCenterline, snapToWallCorner, snapToWallEdge, snapToWallEndpoint, snapToWallMidpoint, stairHalfExtents,
...
export default function Canvas2D({
  ...
  const snapWithInfo = useCallback((p: Vec2, drawingStart?: Vec2): { point: Vec2; feature: boolean } => {
    ...
    // Anywhere along a wall CENTERLINE — so a drawn wall (e.g. a garage wall
    // coming back to the shell) joins the existing wall the way every other
    // junction does. Centerline (not face) contact is what T-junction noding,
    // the roof footprint tracer and mitered rendering all key off.
    s = snapToWallCenterline(q, level.walls, TOL);
    if (s !== q) return { point: s, feature: true };
