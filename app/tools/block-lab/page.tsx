...
// ─── Visual block stacks in lesson notes ─────────────────────────────────────
// :::blocks fences render REAL Blockly (read-only, zelos, same dark theme and
// scale as the editor) so the notes look identical to the coding window.
...
function LessonPanel({ text, itemName }: { text: string; itemName?: string }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const raw = lines[i];
    // :::blocks fences (not ``` — notes live inside template literals)
    if (raw.trim() === ':::blocks') {
      flushTable();
      const buf: string[] = [];
      i++;
