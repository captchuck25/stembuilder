...
// GET /api/student/tower-assignments
// Returns tower assignments for all classes the student is enrolled in,
...
export async function GET() {
  ...
  const [{ data: assignments }, { data: submissions }] = await Promise.all([
    db.from('tower_assignments')
      .select('*, classes(name)')
      .in('class_id', classIds)
      .order('created_at', { ascending: false }),
    db.from('tower_submissions')
