...
export async function POST(req: NextRequest) {
  ...
  const designId = await findOwnDesign(db, 'bridge_designs', session.user.id, assignmentId)
  await recordAssignmentCompletion(db, {
    studentId: session.user.id, tool: 'bridge', assignmentId,
    title: assignment.title ?? 'Bridge Assignment', classId: assignment.class_id,
    passed: !!passed,
    summary: `$${Math.round(Number(cost) || 0).toLocaleString()} · ${passed ? 'passed' : 'not passed'}`,
    result: { cost, maxCost: assignment.max_cost },
    designRef: designId ? { tool: 'bridge', id: designId } : null,
  })
