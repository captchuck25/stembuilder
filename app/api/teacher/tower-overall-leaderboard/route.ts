...
// GET /api/teacher/tower-overall-leaderboard
// Overall standings (best passing submission per student) plus per-assignment
// standings across all the teacher's classes. Same-titled assignments merge.
export async function GET() {
  ...
  for (const a of assignments ?? []) titleById[a.id] = a.title || 'Tower Challenge'
  ...
  const overall: LeaderboardRow[] = [...bestByStudent.entries()]
    ...
    .map(([student_id, best], i) => ({
      ...
      cost: best.cost,
      assignment_title: titleById[best.assignment_id] ?? 'Tower Challenge',
    }))

  const byTitle = new Map<string, Map<string, number>>()
  for (const s of submissions) {
    const title = titleById[s.assignment_id] ?? 'Tower Challenge'
